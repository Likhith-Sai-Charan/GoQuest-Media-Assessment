...
const DubbingInterface = () => {
  ...
  return (
    <DubbingContext.Provider value={{ currentDialogue, isRecording, audioUrl, error }}>
      <div className="flex flex-col h-screen bg-gray-100 p-4">
        ...
        <div className="flex justify-center space-x-4 mb-4">
          <button onClick={isVideoPlaying ? pauseVideo : playVideo} className="p-2 bg-blue-500 text-white rounded-full">
            {isVideoPlaying ? '⏸️ Pause' : '▶️ Play'}
          </button>
          <button onClick={() => seekVideo(-10)} className="p-2 bg-gray-500 text-white rounded-full">⏪ -10s</button>
          <button onClick={() => seekVideo(10)} className="p-2 bg-gray-500 text-white rounded-full">⏩ +10s</button>
        </div>

        {/* Dialogue Navigation */}
        <div className="flex justify-center space-x-4 mb-4">
          <button onClick={prevDialogue} className="p-2 bg-yellow-500 text-white rounded-full">Previous</button>
          <button onClick={nextDialogue} className="p-2 bg-green-500 text-white rounded-full">Next</button>
        </div>
        ...
        {/* Audio recorder */}
        <div className="flex justify-center space-x-4 mb-4">
          <button onClick={isRecording ? stopRecording : startRecording} className="p-2 bg-red-500 text-white rounded-full">
            {isRecording ? '⏹️' : '🎙️'}
          </button>
        </div>
